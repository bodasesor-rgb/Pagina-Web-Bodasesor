...
export default function VajillaDetailPage({ slug }: Props) {
  ...
  const idx = VAJILLAS.findIndex(v => v.slug === product.slug);
  const prev = idx > 0 ? VAJILLAS[idx - 1] : null;
  const next = idx < VAJILLAS.length - 1 ? VAJILLAS[idx + 1] : null;

  const sameCat = VAJILLAS.filter(v => v.cat === product.cat && v.slug !== product.slug);
  const others = VAJILLAS.filter(v => v.cat !== product.cat).slice(0, 6);
      ...
      <section className="bg-[#162040]">
        <div className="max-w-7xl mx-auto px-4 py-10 lg:py-0 grid lg:grid-cols-5 min-h-[440px] items-center">
          ...
          <div className="lg:col-span-2 py-10 lg:py-14 lg:pr-8">
            ...
            <Link href="/vajillas" className="text-white/40 hover:text-white/70 font-serif text-xs flex items-center gap-1.5 transition-colors">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {product.piezas.map(pieza => (
              <div key={pieza}
                ...
                <span className="font-serif text-sm text-[#162040]">{pieza}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-8 px-4 bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#f5efe8] rounded-2xl px-6 py-5">
          <div>
            ...
            <p className="font-serif text-sm text-gray-500">Explora todas las colecciones de vajillas y estilo</p>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {sameCat.map(v => (
                <Link key={v.slug} href={`/vajillas/${v.slug}`}
                  ...
                  <div className="p-3">
                    <p className="text-xs font-serif font-semibold text-[#162040] leading-snug">{v.name}</p>
                  </div>
                </Link>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              {others.map(v => (
                <Link key={v.slug} href={`/vajillas/${v.slug}`}
                  ...
                  <div className="p-2.5">
                    <p className="text-xs font-serif font-semibold text-[#162040] leading-snug">{v.name}</p>
                    <p className="text-[10px] font-serif text-gray-400 mt-0.5">{v.catLabel}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="bg-[#162040] py-10 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-white/70 font-serif mb-6">
            Cotiza tu {product.name} con nosotros. Amplio inventario, entrega incluida y asesoría personalizada.
          </p>
