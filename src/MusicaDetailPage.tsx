...
export default function MusicaDetailPage({ slug }: { slug?: string }) {
  const { city } = useCity();
  const product = MUSICA.find(p => p.slug === slug);
  if (!product) return <div className="min-h-screen flex items-center justify-center font-serif text-[#162040]">Opción no encontrada.</div>;

  const others = MUSICA.filter(p => p.slug !== slug).slice(0, 3);
    ...
    <div className="min-h-screen bg-white">
      <div className="bg-[#f5efe8] border-b border-[#162040]/10 py-3 px-4">
        <div className="max-w-7xl mx-auto flex items-center gap-2 font-serif text-xs text-[#162040]/50">
          ...
          <Link href="/musica" className="hover:text-[#162040]">Música</Link><span>/</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid lg:grid-cols-2 gap-12 mb-16">
          ...
          <div>
            <p className="text-[10px] font-serif font-bold uppercase tracking-widest text-[#162040]/40 mb-2">Música para Eventos</p>
        ...
        {others.length > 0 && (
          <div>
            <h2 className="text-2xl font-serif font-bold text-[#162040] mb-6">Otras opciones musicales</h2>
