...
const CAT_ICONS: Record<PistaTarimaCat, string> = {
  tarimas: '🪵',
  pistas: '💃',
  escenarios: '🎭',
  sets: '✨',
...
export default function PistaTarimaDetailPage({ slug }: Props) {
  const { city } = useCity();
  ...
  const idx = PISTAS_TARIMAS.findIndex(p => p.slug === product.slug);
  const prev = idx > 0 ? PISTAS_TARIMAS[idx - 1] : null;
  const next = idx < PISTAS_TARIMAS.length - 1 ? PISTAS_TARIMAS[idx + 1] : null;

  const sameCat = PISTAS_TARIMAS.filter(p => p.cat === product.cat && p.slug !== product.slug);
  const others = PISTAS_TARIMAS.filter(p => p.cat !== product.cat).slice(0, 6);
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 py-10 lg:py-0 grid lg:grid-cols-5 min-h-[440px] items-center">
          ...
          <div className="lg:col-span-2 py-10 lg:py-14 lg:pr-8">
            <nav className="flex items-center gap-2 text-xs text-white/50 mb-6 font-serif flex-wrap">
              ...
              <span>/</span>
              <Link href="/pistas-tarimas" className="hover:text-white transition-colors">Pistas y Tarimas</Link>
              <span>/</span>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {product.features.map(f => (
              <div key={f} className="flex items-start gap-3 bg-white rounded-xl px-4 py-3">
                ...
                <span className="font-serif text-sm text-[#162040]">{f}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-8 px-4 border-b border-gray-100">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <p className="font-serif font-bold text-[#162040]">¿Quieres ver el catálogo completo?</p>
            <p className="font-serif text-sm text-gray-500">Explora todas las pistas y tarimas disponibles</p>
          </div>
      ...
      <section className="py-8 px-4 border-b border-gray-100">
        <div className="max-w-5xl mx-auto flex justify-between items-center gap-4">
          {prev ? (
            <Link href={`/pistas-tarimas/${prev.slug}`}
          ...
          {next ? (
            <Link href={`/pistas-tarimas/${next.slug}`}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              {others.map(p => (
                <Link key={p.slug} href={`/pistas-tarimas/${p.slug}`}
                  ...
                  <div className="p-2.5">
                    <p className="text-xs font-serif font-semibold text-[#162040] leading-snug">{p.name}</p>
                    <p className="text-[10px] font-serif text-gray-400 mt-0.5">{p.catLabel}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="bg-[#162040] py-10 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-white/70 font-serif mb-6">
            Cotiza tu {product.name} con nosotros. Instalación profesional, materiales premium y personalización total.
          </p>
