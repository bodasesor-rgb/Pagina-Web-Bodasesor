    ...
    <div className="min-h-screen bg-white">
      <section className="bg-[#162040] text-white">
        <div className="max-w-7xl mx-auto px-4 py-16 lg:py-24 grid lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="text-white/50 font-serif text-sm mb-8">{EMPRESAS.length} servicios disponibles. Para empresas de cualquier tamaño, desde 10 hasta miles de colaboradores.</p>
        ...
        <div className="border-t border-white/10 py-4">
          <div className="max-w-7xl mx-auto px-4 flex gap-6 text-white/40 font-serif text-xs">
            <span>Normas HACCP</span><span>•</span><span>Proveedores certificados</span><span>•</span><span>Facturación empresarial</span>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-10">
            ...
            <div>
              <p className="text-[10px] font-serif font-bold uppercase tracking-widest text-[#162040]/40">Catálogo</p>
              <h2 className="text-2xl font-serif font-bold text-[#162040]">Servicios de Alimentación Empresarial</h2>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {EMPRESAS.map(p => <EmpresaCard key={p.slug} product={p} />)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#162040] py-14 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-3xl font-serif font-bold text-white mb-4">¿Qué servicio necesita tu empresa?</h2>
          <p className="text-white/70 font-serif mb-8">Cuéntanos el número de colaboradores y el tipo de servicio que buscas y te preparamos una propuesta en 24 horas.</p>
          <a href={waGeneral} target="_blank" rel="noopener noreferrer"
...
function EmpresaCard({ product }: { product: EmpresasProduct }) {
  const waMsg = WA_BASE + encodeURIComponent(`Hola, me interesa el servicio "${product.name}" para mi empresa.`);
  return (
    <div className="group bg-white rounded-2xl overflow-hidden border border-[#162040]/8 hover:border-[#162040]/25 hover:shadow-xl transition-all duration-300">
      <Link href={`/alimentos-empresas/${product.slug}`}>
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2">
          <Link href={`/alimentos-empresas/${product.slug}`} className="flex-1 text-center text-xs font-serif font-semibold text-[#162040] border border-[#162040]/20 py-2 rounded-lg hover:bg-[#f5efe8] transition-colors">Ver detalle</Link>
          <a href={waMsg} target="_blank" rel="noopener noreferrer" className="flex-1 text-center text-xs font-serif font-semibold text-white bg-[#25D366] hover:bg-green-500 py-2 rounded-lg transition-colors">Cotizar</a>
        </div>
      </div>
    </div>
  );
