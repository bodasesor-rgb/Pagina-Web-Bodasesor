...
export default function EmpresasDetailPage({ slug }: { slug?: string }) {
  const { city } = useCity();
  const product = EMPRESAS.find(p => p.slug === slug);
  ...
  const others = EMPRESAS.filter(p => p.slug !== slug).slice(0, 3);
    ...
    <div className="min-h-screen bg-white">
      <div className="bg-[#f5efe8] border-b border-[#162040]/10 py-3 px-4">
        <div className="max-w-7xl mx-auto flex items-center gap-2 font-serif text-xs text-[#162040]/50">
          ...
          <Link href="/alimentos-empresas" className="hover:text-[#162040]">Alimentos para Empresas</Link><span>/</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid lg:grid-cols-2 gap-12 mb-16">
          ...
          <div>
            <p className="text-[10px] font-serif font-bold uppercase tracking-widest text-[#162040]/40 mb-2">Alimentos para Empresas</p>
        ...
        {others.length > 0 && (
          <div>
            <h2 className="text-2xl font-serif font-bold text-[#162040] mb-6">Otros servicios para empresas</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {others.map(p => (
                <Link key={p.slug} href={`/alimentos-empresas/${p.slug}`} className="group bg-white rounded-2xl overflow-hidden border border-[#162040]/8 hover:border-[#162040]/25 hover:shadow-lg transition-all">
                  ...
                  <div className="p-4"><h3 className="font-serif font-bold text-[#162040] text-sm mb-1">{p.name}</h3><p className="font-serif text-gray-400 text-xs italic">{p.tagline}</p></div>
                </Link>
              ))}
            </div>
          </div>
