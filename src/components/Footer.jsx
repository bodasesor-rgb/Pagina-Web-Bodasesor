import { useLocation } from "wouter";
import CityLink from "./CityLink";
import { NEXUS_PRIORITY_LINKS } from "../data/nexus-priority-links";

const WHATSAPP_NUMBER = "5215540080373";

const Link = CityLink;

/** Nexus landings — plain <a>, never CityLink (glued city slugs break if city is appended). */
const NEXUS_FOOTER_LINKS = NEXUS_PRIORITY_LINKS.slice(0, 6).map((l) => ({
  name: l.shortLabel || l.label,
  href: l.href,
}))

const CITIES = [
  { name: 'Ciudad de México', href: '/ciudad-de-mexico' },
  { name: 'Guadalajara',       href: '/guadalajara' },
  { name: 'Monterrey',         href: '/monterrey' },
  { name: 'Querétaro',         href: '/queretaro' },
  { name: 'Puebla',            href: '/puebla' },
  { name: 'Cancún',            href: '/cancun' },
  { name: 'Mérida',            href: '/merida' },
]

const linkClass = "text-white/80 hover:text-white hover:font-bold hover:translate-x-1 inline-block transition-all duration-200 font-serif"

export default function Footer() {
  const [, setLocation] = useLocation()
  return (
    <footer className="bg-[#162040] text-white pt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">

          {/* Lo Más Buscado */}
          <div>
            <p className="text-xl font-bold mb-6 font-serif">Lo Más Buscado</p>
            <ul className="space-y-3">
              {[
                { name: 'Catálogos', href: '/catalogos' },
                { name: 'Banquetes para Bodas', href: '/banquetes' },
                { name: 'Barra de Bebidas', href: '/barras-de-bebidas' },
                { name: 'Renta de Mobiliario', href: '/mesas-sillas' },
                { name: 'Wedding Planner', href: '/wedding-planner' },
              ].map(item => (
                <li key={item.href}>
                  <Link href={item.href} className={linkClass}>{item.name}</Link>
                </li>
              ))}
              {NEXUS_FOOTER_LINKS.map(item => (
                <li key={item.href}>
                  <a href={item.href} className={linkClass}>{item.name}</a>
                </li>
              ))}
            </ul>
          </div>

          {/* Bodasesor */}
          <div>
            <p className="text-xl font-bold mb-6 font-serif">Bodasesor</p>
            <ul className="space-y-3">
              {[
                { name: 'Banquetes & Catering', href: '/banquetes-catering' },
                { name: 'Barras de Bebidas', href: '/barras-de-bebidas' },
                { name: 'Mesas Personalizadas', href: '/mesas-personalizadas' },
                { name: 'Mobiliario Premium', href: '/salas-periqueras' },
                { name: 'Wedding Planner', href: '/wedding-planner' },
                { name: 'Blog de Eventos', href: '/blog' },
                { name: 'Quiénes Somos', href: '/quienes-somos' },
                { name: 'Aviso de Privacidad', href: '/aviso-de-privacidad' },
                { name: 'Términos y Condiciones', href: '/terminos-y-condiciones' },
                { name: 'Políticas de Devoluciones', href: '/politicas-de-devoluciones' },
              ].map(item => (
                <li key={item.href}>
                  <Link href={item.href} className={linkClass}>{item.name}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Ciudades */}
          <div>
            <p className="text-xl font-bold mb-6 font-serif">Ciudades</p>
            <ul className="space-y-3">
              {CITIES.map(item => (
                <li key={item.href}>
                  <button
                    onClick={() => {
                      setLocation(item.href);
                      window.scrollTo({ top: 0, behavior: 'smooth' });
                    }}
                    className={linkClass + ' bg-transparent border-0 p-0 cursor-pointer text-left'}
                  >
                    {item.name}
                  </button>
                </li>
              ))}
            </ul>
          </div>

          {/* Contacto */}
          <div>
            <p className="text-xl font-bold mb-6 font-serif">Contacto</p>
            <ul className="space-y-4">
              <li>
                <a href={`https://api.whatsapp.com/send?phone=${WHATSAPP_NUMBER}`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center text-white/80 hover:text-white hover:font-bold hover:translate-x-1 transition-all duration-200 font-serif group">
                  <svg className="w-5 h-5 mr-3 group-hover:scale-125 transition-transform duration-200" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                  </svg>
                  WhatsApp
                </a>
              </li>
              <li>
                <a href="tel:5215540080373"
                  className="flex items-center text-white/80 hover:text-white hover:font-bold hover:translate-x-1 transition-all duration-200 font-serif group">
                  <svg className="w-5 h-5 mr-3 group-hover:scale-125 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                  554 008 0373
                </a>
              </li>
              <li>
                <a href="mailto:hola@bodasesor.com"
                  className="flex items-center text-white/80 hover:text-white hover:font-bold hover:translate-x-1 transition-all duration-200 font-serif group">
                  <svg className="w-5 h-5 mr-3 group-hover:scale-125 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  hola@bodasesor.com
                </a>
              </li>
              <li className="pt-2">
                <p className="text-white/60 text-sm font-serif">Lunes a Sábado<br />9:00 - 18:00 hrs</p>
              </li>
            </ul>
          </div>
        </div>
      </div>

      <div className="bg-white border-t border-[#162040]/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col md:flex-row justify-between items-center">
          <p className="text-[#162040]/70 text-sm font-serif mb-4 md:mb-0">
            © {new Date().getFullYear()} Bodasesor. Todos los derechos reservados.
          </p>
          <div className="flex space-x-6">
            <a href="https://www.facebook.com/profile.php?id=61551880049993"
              target="_blank" rel="noopener noreferrer"
              aria-label="Facebook de Bodasesor"
              className="text-[#162040] hover:text-[#1a2a52] hover:scale-125 transition-all duration-200">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
              </svg>
            </a>
            <a href="https://www.instagram.com/bodasesormx/"
              target="_blank" rel="noopener noreferrer"
              aria-label="Instagram de Bodasesor"
              className="text-[#162040] hover:text-[#1a2a52] hover:scale-125 transition-all duration-200">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
              </svg>
            </a>
          </div>
        </div>
      </div>
    </footer>
  )
}
