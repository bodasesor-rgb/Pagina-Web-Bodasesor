    ...
    <>
      <section className="pt-32 pb-16 px-4 bg-gray-900 text-white text-center">
        <div className="max-w-3xl mx-auto">
          <p className="text-amber-400 text-sm font-semibold uppercase tracking-widest mb-3">Bebidas para eventos</p>
          ...
          <p className="text-amber-400 text-sm mb-8">5 opciones disponibles · Con y sin alcohol · Bartenders certificados · Montaje incluido</p>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-wrap gap-3 justify-center mb-10">
            {BARRAS_SERVICES.map(s => (
              <span key={s.slug} className="px-4 py-2 bg-white border border-amber-200 text-amber-700 text-sm rounded-full">{s.icon} {s.name}</span>
            ))}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {BARRAS_SERVICES.map((s) => (
              <div key={s.slug} className="bg-white rounded-2xl p-7 shadow-sm border border-gray-100 hover:shadow-md hover:border-amber-200 transition-all">
