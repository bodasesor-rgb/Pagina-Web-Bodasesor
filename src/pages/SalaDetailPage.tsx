...
export default function SalaDetailPage({ salaSlug }: Props) {
  ...
  const prev = idx > 0 ? SALAS_CATALOG[idx - 1] : null;
  const next = idx < SALAS_CATALOG.length - 1 ? SALAS_CATALOG[idx + 1] : null;
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 py-10 lg:py-0 grid lg:grid-cols-5 min-h-[420px] items-center">
          ...
          <div className="lg:col-span-2 py-10 lg:py-14 lg:pr-8">
            <nav className="flex items-center gap-2 text-xs text-white/50 mb-6 font-serif flex-wrap">
              ...
              <span>/</span>
              <span className="text-white/80">Salas</span>
            </nav>
      ...
      <section className="py-10 px-4 bg-[#f5efe8]">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-serif font-bold text-[#162040] mb-5">Otras salas disponibles</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {SALAS_CATALOG.filter(s => s.slug !== sala.slug).slice(0, 6).map(s => (
