...
export default function CombinacionDetailPage({ slug }: Props) {
  const { city } = useCity();
  ...
  const waMsg = WA_BASE + encodeURIComponent(`Hola, me interesa cotizar la combinación: ${combo.label} para mi evento.`);
  const idx = COMBINACIONES.findIndex(c => c.slug === combo.slug);
  const prev = idx > 0 ? COMBINACIONES[idx - 1] : null;
  const next = idx < COMBINACIONES.length - 1 ? COMBINACIONES[idx + 1] : null;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-[#162040]">
        <div className="max-w-7xl mx-auto px-4 py-10 lg:py-0 grid lg:grid-cols-5 min-h-[440px] items-center">
          ...
          <div className="lg:col-span-2 py-10 lg:py-14 lg:pr-8">
            <nav className="flex items-center gap-2 text-xs text-white/50 mb-6 font-serif flex-wrap">
              ...
              <span>/</span>
              <Link href="/combinaciones-mesas-sillas" className="hover:text-white transition-colors">Combinaciones</Link>
            </nav>

            <h1 className="text-3xl md:text-4xl font-serif font-bold text-white leading-tight mb-4">
              {combo.label}{city ? ` en ${city.name}` : ''}
            </h1>
            <p className="text-white/80 font-serif leading-relaxed mb-4 text-lg">
              {combo.desc}
            </p>
            <p className="text-white/60 font-serif leading-relaxed mb-8 text-sm">
              {combo.detail}
            </p>
      ...
      <section className="py-8 px-4 border-b border-gray-100">
        <div className="max-w-5xl mx-auto flex justify-between items-center gap-4">
          {prev ? (
            <Link href={`/combinaciones/${prev.slug}`}
          ...
          {next ? (
            <Link href={`/combinaciones/${next.slug}`}
      ...
      {/* Other combinations grid */}
      <section className="py-12 px-4 bg-[#f5efe8]">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-xl font-serif font-bold text-[#162040] mb-6">Otras combinaciones disponibles</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            {COMBINACIONES.filter(c => c.slug !== combo.slug).map(c => (
              <Link key={c.slug} href={`/combinaciones/${c.slug}`}
                ...
                <div className="p-2.5">
                  <p className="text-xs font-serif font-semibold text-[#162040] leading-snug">{c.label}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="bg-[#162040] py-10 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-2xl font-serif font-bold text-white mb-3">
            ¿Buscas una combinación diferente?
          </h2>
          <p className="text-white/70 font-serif mb-6">
            Tenemos más de 18 modelos de sillas y 6 tipos de mesas. Podemos crear la combinación perfecta para tu evento.
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <a href={WA_BASE + encodeURIComponent('Hola, me gustaría cotizar una combinación personalizada de mesas y sillas.')}
              ...
              <WaSvg /> Consultar combinación personalizada
            </a>
            <Link href="/combinaciones-mesas-sillas"
