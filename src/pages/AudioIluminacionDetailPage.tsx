...
export default function AudioIluminacionDetailPage({ slug }: { slug?: string }) {
  ...
  const others = AUDIO_ILUMINACION.filter(p => p.slug !== slug && p.category === product.category).slice(0, 3);
  const waMsg = WA_BASE + encodeURIComponent(`Hola, me interesa cotizar "${product.name}" para mi evento.`);
  const catLabel = CATEGORY_LABEL[product.category] ?? 'Audio, Iluminación y Video';
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-[#f5efe8] border-b border-[#162040]/10 py-3 px-4">
        <div className="max-w-7xl mx-auto flex items-center gap-2 font-serif text-xs text-[#162040]/75">
          ...
          <span>/</span>
          <Link href={`/audio-iluminacion-video#${product.category}`} className="hover:text-[#162040]">{catLabel}</Link>
          <span>/</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {others.length > 0 && (
          <div>
            <h2 className="text-2xl font-serif font-bold text-[#162040] mb-6">
              Más servicios de {CATEGORY_LABEL[product.category]}
            </h2>
        ...
        <div className="mt-12 text-center">
          <Link href="/audio-iluminacion-video"
            ...
            ← Ver Catálogocompleto de Audio, Iluminación y Video
          </Link>
        </div>
      </div>
    </div>
