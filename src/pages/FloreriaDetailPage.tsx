...
export default function FloreriaDetailPage({ slug }: Props) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f5efe8]">
        <div className="text-center">
          <p className="text-2xl font-serif text-[#162040] mb-4">Producto no encontrado</p>
          <Link href="/floreria" className="text-[#162040] underline font-serif">Ver toda la florería y decoración</Link>
        </div>
      </div>
    );
  }
  ...
  const categoryIcon = product.category === 'floral' ? '✿' : product.category === 'globos' ? '○' : '✦';
    ...
    <div className="min-h-screen bg-white">
        ...
        <section className="py-10 px-4 bg-[#f5efe8]">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {sameCategory.map(p => (
                <Link key={p.slug} href={`/floreria/${p.slug}`}
      ...
      <section className="bg-[#162040] py-10 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-2xl font-serif font-bold text-white mb-3">¿Listo para cotizar?</h2>
          <p className="text-white/70 font-serif mb-6">
            Cotiza tu {product.name} con nosotros. Diseño personalizado e instalación profesional incluida.
          </p>
