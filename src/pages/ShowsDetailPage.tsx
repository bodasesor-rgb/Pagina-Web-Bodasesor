...
export default function ShowsDetailPage({ slug }: Props) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f5efe8]">
        <div className="text-center">
          <p className="text-2xl font-serif text-[#162040] mb-4">Show no encontrado</p>
          <Link href="/shows" className="text-[#162040] underline font-serif">Ver todos los shows</Link>
        </div>
      </div>
    );
  }
  ...
  const sameCategory = SHOWS_BY_CATEGORY[product.category].filter(p => p.slug !== product.slug);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-[#162040]">
        <div className="max-w-7xl mx-auto px-4 py-10 lg:py-0 grid lg:grid-cols-5 min-h-[480px] items-center">
          ...
          <div className="lg:col-span-2 py-10 lg:py-14 lg:pr-8">
            ...
            <p className="text-white/60 font-serif text-sm italic mb-4">{product.tagline}</p>

            <div className="flex items-center gap-2 mb-4">
              <svg className="w-4 h-4 text-[#8a9bb5] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="font-serif text-white/60 text-sm">Duración: <strong className="text-white/80">{product.duracion}</strong></span>
            </div>

            <p className="text-white/80 font-serif leading-relaxed mb-3 text-base">{product.short}</p>
      ...
      <section className="py-8 px-4 bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#f5efe8] rounded-2xl px-6 py-5">
          <div>
            <p className="font-serif font-bold text-[#162040]">¿Quieres ver todos los shows?</p>
            <p className="font-serif text-sm text-gray-600">Explora el catálogo completo de entretenimiento</p>
          </div>
          <Link href="/shows"
      ...
      <section className="py-8 px-4 border-b border-gray-100">
        <div className="max-w-5xl mx-auto flex justify-between items-center gap-4">
          ...
          <Link href="/shows" className="text-xs font-serif font-semibold text-[#162040]/75 hover:text-[#162040] uppercase tracking-wide whitespace-nowrap">
            Ver catálogo
          </Link>
          {next ? (
            <Link href={`/shows/${next.slug}`}
              ...
              {sameCategory.map(p => (
                <Link key={p.slug} href={`/shows/${p.slug}`}
                  ...
                  <div className="p-4">
                    ...
                    <p className="font-serif text-xs text-gray-600 mt-0.5">{p.duracion}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="bg-[#162040] py-10 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-2xl font-serif font-bold text-white mb-3">¿Listo para contratar?</h2>
          <p className="text-white/70 font-serif mb-6">
            Cotiza el {product.name} con nosotros. También podemos combinar varios shows para tu evento.
          </p>
