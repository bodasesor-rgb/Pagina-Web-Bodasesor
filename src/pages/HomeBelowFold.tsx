...
// ─── Gallery carousel ─────────────────────────────────────────────────────────
// 60 fotos reales (sin flyers de publicidad)
const HOME_GALLERY_ALT_POOL = [
...
const HOME_GALLERY_IDS = Array.from({ length: 281 }, (_, i) => i + 1)
  .filter((n) => !INSTAGRAM_AD_EXCLUDES.has(n))
  .slice(0, 60);
const gallerySlides = [];
const galleryAlts = [];
for (let i = 0; i < HOME_GALLERY_IDS.length; i += 3) {
  const chunk = HOME_GALLERY_IDS.slice(i, i + 3);
  if (chunk.length < 3) break;
  gallerySlides.push(chunk.map((n) => `/images/instagram/ig${n}.jpg`));
  galleryAlts.push(chunk.map((_, j) => HOME_GALLERY_ALT_POOL[(i + j) % HOME_GALLERY_ALT_POOL.length]));
}
...
function TestimonialsCarousel() {
  ...
  const visible = [0, 1, 2].map((offset) => testimonials[(idx + offset) % total]);
  ...
  return (
    <div className="relative">
      <button
        ...
        className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-[#162040] hover:bg-[#1a2a52] text-white p-2.5 rounded-full shadow-lg -ml-2 md:-ml-4"
      >
        <ChevronLeft className="w-5 h-5" aria-hidden="true" />
      </button>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 px-8 md:px-10">
        {visible.map((t, i) => (
          <div
            key={`${t.name}-${idx}-${i}`}
          ...
          >
            ...
            <div className="border-t border-[#162040]/15 pt-3">
              <p className="font-bold text-[#162040] font-serif">{t.name}</p>
              <p className="text-gray-600 font-serif text-sm">{t.type}</p>
            </div>
          </div>
        ))}
      </div>
      <button
        ...
        className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-[#162040] hover:bg-[#1a2a52] text-white p-2.5 rounded-full shadow-lg -mr-2 md:-mr-4"
      >
        <ChevronRight className="w-5 h-5" aria-hidden="true" />
      </button>
      <div className="flex justify-center gap-1.5 mt-5" role="tablist" aria-label="Reseñas">
        {testimonials.map((t, i) => (
          <button
            ...
            onClick={() => setIdx(i)}
            className={`h-2 rounded-full transition-all ${i === idx ? 'w-6 bg-[#162040]' : 'w-2 bg-[#162040]/30'}`}
          />
        ))}
      </div>
    </div>
...
interface HomeBelowFoldProps {
  city?: { name: string; slug: string; short?: string } | null;
}

export default function HomeBelowFold({ city: cityProp }: HomeBelowFoldProps) {
  const { city: cityCtx } = useCity();
  const city = cityProp ?? cityCtx;
  ...
  return (
    <>
      {/* ── Stats (trust badges) ── */}
      <section id="trust-badges" className="bg-[#f5efe8] py-7 relative overflow-hidden cv-auto" data-testid="section-stats">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 items-stretch text-center">
            <StatBlock delay={0}>
              ...
              <div className="text-2xl md:text-3xl font-bold text-[#162040] mb-0.5 font-serif">1,000+</div>
              <div className="text-gray-700 font-medium text-xs md:text-sm font-serif">Eventos realizados</div>
            </StatBlock>
            <StatBlock delay={150}>
              <div className="flex justify-center mb-1.5">
                <svg className="w-7 h-7 md:w-8 md:h-8 text-[#0d6849]" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="text-2xl md:text-3xl font-bold text-[#162040] mb-0.5 font-serif">10,000+</div>
              <div className="text-gray-700 font-medium text-xs md:text-sm font-serif">Personas atendidas</div>
            </StatBlock>
            <StatBlock delay={300}>
              <div className="flex justify-center mb-1.5">
                <svg className="w-7 h-7 md:w-8 md:h-8 text-[#0d6849]" fill="currentColor" viewBox="0 0 24 24">
      ...
      <section id="servicios" className="py-12 md:py-14 bg-white" data-testid="section-servicios">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="text-3xl md:text-4xl font-bold text-[#162040] mb-3 font-serif">Banquetes, Catering y Mobiliario para Tu Evento</h2>
            <p className="text-base md:text-lg text-gray-600 max-w-3xl mx-auto font-serif">Todo lo que necesitas para bodas, XV años y eventos corporativos en un solo lugar</p>
          </div>
      ...
      {/* ── Galería ── */}
      <section id="galeria" className="py-12 md:py-14 bg-[#f5efe8]" data-testid="section-gallery">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="text-3xl md:text-4xl font-bold text-[#162040] mb-3 font-serif">Bodas y Banquetes Reales Que Hemos Producido</h2>
            <p className="text-base md:text-lg text-gray-800 max-w-3xl mx-auto font-serif">Descubre la magia que creamos en cada celebración</p>
          </div>
          <GalleryCarousel />
          <div className="text-center mt-8">
            <a
              ...
              className="inline-block bg-[#162040] hover:bg-[#1a2a52] text-white px-8 py-3 rounded-lg font-bold transition-colors duration-300 font-serif"
            >
              Ver Más Eventos
            </a>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
            {cities.map(c => (
              <button
                ...
                className="bg-[#eceef2] hover:bg-[#162040] hover:text-white text-[#162040] font-serif font-semibold py-2.5 px-4 rounded-lg text-center text-sm transition-all duration-300 cursor-pointer"
              >
                {c.name}
              </button>
            ))}
      ...
      {/* ── Cómo funciona ── */}
      <section className="py-10 md:py-12 bg-[#f5efe8]" data-testid="section-como-funciona">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="text-3xl md:text-4xl font-bold text-[#162040] mb-2 font-serif">Cómo Funciona</h2>
            <p className="text-base text-gray-800 max-w-3xl mx-auto font-serif">Organizar tu evento es más fácil de lo que piensas</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={step.title} className="text-center bg-[#eceef2] rounded-xl p-4 md:p-5">
                ...
                <h3 className="text-lg font-bold text-[#162040] mb-1.5 font-serif">{step.title}</h3>
                <p className="text-gray-700 font-serif text-sm leading-snug">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-center">
            <div>
              ...
              <p className="text-base text-gray-700 font-serif leading-relaxed mb-5">
              ...
              <div className="flex gap-8 mb-6">
                <div>
                  <div className="text-2xl font-bold text-[#162040] font-serif">+10</div>
                  <div className="text-gray-600 font-serif text-xs">años de experiencia</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-[#162040] font-serif">100%</div>
                  <div className="text-gray-600 font-serif text-xs">atención personalizada</div>
                </div>
              </div>
              <Link href="/quienes-somos" className="inline-block bg-[#162040] hover:bg-[#1a2a52] text-white px-6 py-3 rounded-lg font-bold font-serif transition-all duration-300 hover:scale-105">
      ...
      <section className="py-10 bg-[#162040] overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mb-6">
          <p className="text-[#f5efe8] text-xs font-serif tracking-widest mb-2 uppercase">Nuestros clientes</p>
          <h2 className="text-2xl md:text-3xl font-bold text-white font-serif">Empresas Que Confían en Nosotros</h2>
        </div>
      ...
      {/* ── Testimonios ── */}
      <section id="testimonios" className="py-10 md:py-12 bg-[#f5efe8]" data-testid="section-reviews">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-6">
            <h2 className="text-3xl md:text-4xl font-bold text-[#162040] mb-2 font-serif">Lo Que Dicen Nuestros Clientes</h2>
            <p className="text-base text-gray-600 max-w-3xl mx-auto font-serif">Testimonios reales de quienes confiaron en nosotros</p>
          </div>
          <TestimonialsCarousel />
          <div className="text-center mt-8">
            <p className="text-gray-700 mb-4 font-serif text-base flex items-center justify-center gap-2 flex-wrap">
              Calificación promedio: <span className="font-bold text-[#162040] text-xl">4.6/5</span> en
              <svg viewBox="0 0 272 92" className="h-6 inline-block" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
              ...
              <a
                ...
                target="_blank" rel="noopener noreferrer"
                className="inline-block bg-white hover:bg-[#f5efe8] text-[#162040] border-2 border-[#162040] px-8 py-3 rounded-lg font-bold transition-colors duration-300 font-serif"
              >
                Escribe Una Reseña
              </a>
            </div>
      ...
      {/* ── CTA final ── */}
      <section className="bg-[#162040] py-12 md:py-14" data-testid="section-cta">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-4 font-serif">¿Listo para planear tu evento?</h2>
          <p className="text-lg md:text-xl text-[#f5efe8] mb-8 font-serif">Obtén una cotización personalizada en menos de 5 minutos</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-6">
            <a
              href={`${WHATSAPP_BASE}&text=Hola%2C%20me%20gustar%C3%ADa%20cotizar%20un%20evento`}
              target="_blank" rel="noopener noreferrer"
              data-testid="btn-cta-bottom"
              className="bg-[#0d6849] hover:bg-[#0a5740] text-white px-10 py-4 rounded-lg text-lg font-bold transition-all duration-300 transform hover:scale-105 shadow-2xl flex items-center gap-3 font-serif"
            >
            ...
            <a
              ...
              className="bg-white hover:bg-[#f5efe8] text-[#162040] px-10 py-4 rounded-lg text-lg font-bold transition-all duration-300 font-serif"
            >
              Llamar ahora
            </a>
          </div>
          <div className="flex flex-wrap justify-center gap-6 text-sm text-[#f5efe8]">
